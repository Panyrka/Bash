...
Пример:
sh ./smartlogger.sh -c \"top -b -n 1\" -t 3600 -p 5000 -o today
Данная команда бубет записываться раз в 5 секунд на протяжении 3600 секунд, то есть всего будет 720 записей.
...
if [ $WF -gt 0 ]
then
echo "Перезпись из временных файлов раз в" $WF "секунд"
else
...
echo 
echo "Логгирование начнётся через 10 секунд, если что-то неверно, необходимо нажать CTRL+C"
sleep 1
...
echo "Запись в основной файл через" $(($PERIOD/1000)).$(($PERIOD-($PERIOD/1000)*1000)).
